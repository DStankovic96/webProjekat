...
<template>
	<div style="margin-bottom:150px;" id="user-profile-update">
		<div id='main'>
            ...
            <div class="container" v-show="isCoach">
				<h4>Promena podataka</h4>
				<hr>
				<fieldset class="form-group">
					<label>Korisnicko ime</label>
					<input type="text" class="form-control" v-model="profileCoach.username" disabled />
				</fieldset>

				<div v-if='messages.errorFirstName' id='testError' class="alert alert-danger" v-html="messages.errorFirstName"></div>
				<fieldset class="form-group" >
					<label>Ime</label>
					<input type="text" class="form-control" v-model="profileCoach.name" />
				</fieldset>

				<div v-if='messages.errorLastName' class="alert alert-danger" v-html="messages.errorLastName"></div>
				<fieldset class="form-group" >
					<label>Prezime</label>
					<input type="text" class="form-control" v-model="profileCoach.lastname" />
				</fieldset>
				...
				<div class="form-label-group" >
					...
					<br>
					<span >
                        <select style="padding:5px; width:100%;" v-model="profileCoach.gender">
				...
				<button type="button" class="btn btn-lg btn-success" v-on:click='updateProfile(profileCoach)'> Save </button>
				<hr>
            ...
            <div class="container" v-show="isManager">
                ...
                <fieldset class="form-group">
					<label>Objekat</label>
					<input type="text" class="form-control" v-model="profileManager.object" disabled />
				</fieldset>
...
export default {
    ...
    watch:{
        ...
        'selectedDate4' : function(val){
            console.log(this.profileCoach.dateOfBirth);
            this.profileCoach.dateOfBirth = this.selectedDate4.toString().substring(4, 15);
            console.log(this.profileCoach.dateOfBirth);
        }
    },
    methods:{
            ...
            //Ako su uneta sva 3 passworda, nema errora za prazna polja, ali se proverava validnost samog unosa
                ...
                if (this.changedPassword.newPassword !== this.changedPassword.newPasswordRepeat) {
                    ...
                    if(this.role === "admin"){
                     ...
                     if(this.role === "coach"){
                        dataService.updateCoach(profile).then(response => {
                            this.messages.successResponse = `<h4>Vas profil je uspesno izmenjen!</h4>`;
                            setTimeout(() => this.messages.successResponse = '', 3000);
                            setTimeout(() => this.$router.push(`/home`), 4000);
                            profile = response.data;
                    ...
                    if(this.role === "manager"){
                            ...
                            console.log("menadzer:" + profile.object + profile.name );
